...
def construct_conditional_covariance_matrix(mask, minX, maxX, minY, maxY, n, variance, lengthscale,
    ...
    #should be a scalar
    print((np.transpose((observed_vector-observed_unconditional_mean))).shape)
    p1 = np.matmul(np.matmul(np.transpose((observed_vector-observed_unconditional_mean)), Sigma22inv),
              (observed_vector - observed_unconditional_mean))
    print(p1.shape)
    p1 = float(p1)
