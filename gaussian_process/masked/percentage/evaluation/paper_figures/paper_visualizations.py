...
def visualize_observed_and_diffusion(lengthscale, variance, figname, n, model_name):
    ...
    fig.text(x = .4, y = .95, s = "Percentage U-Net", fontsize = 25)
    plt.tight_layout()
    plt.savefig(figname)



def visualize_observed_and_diffusion_transposed(lengthscale, variance, figname, n, model_name):
    ...
    for i, ax in enumerate(grid):
        ...
        if((i % 3) == 0):
            counter = counter + 1
            im = ax.imshow(reference_visualizations[counter,:,:], cmap='viridis', vmin = -4, vmax = 4,
            ...
            im = ax.imshow(true_visualizations[counter,:,:], cmap='viridis', vmin = -4, vmax = 4)
            ax.set_xticks(ticks = [0, 7, 15, 23, 31], labels = np.array([-10,-5,0,5,10]), fontsize = 15)
            ax.set_yticks(ticks = [0,7,15], labels = np.array([-5,0,5]), fontsize = 15)
            if((i % 2) == 1):
                ax.set_yticks(ticks = [7, 15, 23], labels = np.array([-5,0,5]), fontsize = 15)
            ...
            im = ax.imshow(diffusion_visualizations[counter,:,:], cmap='viridis', vmin = -4, vmax = 4)
            #ax.set_xticks(ticks = [0, 7, 15, 23, 31], labels = np.array([-10,-5,0,5,10]), fontsize = 15)
            #ax.set_yticks(ticks = [0, 7, 15, 23, 31], labels = np.array([-10,-5,0,5,10]), fontsize = 15)
            if((i % 2) == 1):
                ax.set_yticks(ticks = [7, 15, 23], labels = np.array([-5,0,5]), fontsize = 15)
    ...
    cbar = ax.cax.colorbar(im)
    cbar.ax.tick_params(labelsize=15)
    fig.text(x = .25, y = .95, s = "Percentage U-Net", fontsize = 25)
    plt.tight_layout()
...
figname = "figures/gp_percentage_visualization_model7_transposed.png"
visualize_observed_and_diffusion_transposed(lengthscale, variance, figname, n, model_name)