...
def visualize_marginal_bivariate_density(model_name, missing_indices, missing_indices1, missing_indices2, n, nrep, lengthscale, variance, figname):
    ...
    for i in range(0,15):
        ...
        if(i < 5):
            ...
            kde1 = sns.kdeplot(data = pdd, x = 'x', y = 'y', ax = ax, hue = 'class',
                               fill = False, levels = [.1,.2,.3,.4,.5,.6,.7,.8,.9,.95,.99], alpha = .5, legend = False)
...
def visualize_marginal_bivariate_density_transposed(model_name, missing_indices, missing_indices1, missing_indices2, n, nrep, lengthscale, variance, figname):
    ...
    for i in range(0,15):
        ...
        if((i%3)==0):
            ...
            ax.set_ylabel("")
            ax.set_xlabel("")
            #ax.yaxis.tick_right([-4,-2,0,2,4], [-4,-2,0,2,4], fontsize = 15)
            #ax.yaxis.tick_left([])
            ax.set_yticks([-4,-2,0,2,4], [-4,-2,0,2,4], fontsize = 15)
    ...
    fig.text(x = .36, y = .93, s = "Proportion U-Net", fontsize = 15)
    plt.tight_layout()
    plt.savefig(figname, dpi = 500)
    plt.clf()

def visualize_marginal_bivariate_density_with_variables():
    model_name = "model7"
    missing_indices = [838, 700, 200, 301, 495]
    n = 32
    nrep = 4000
    variance = 1.5
    figname = "figures/gp_percentages_conditional_marginal_bivariate_density.png"
    lengthscale = 3.0
    missing_indices1 = [231,426,533,499,450]
    missing_indices2 = [265,430,690,502,490]
    visualize_marginal_bivariate_density(model_name, missing_indices, missing_indices1, missing_indices2, n, nrep, lengthscale, variance, figname)


def visualize_marginal_bivariate_density_transposed_with_variables():
    model_name = "model7"
    missing_indices = [838, 700, 200, 301, 495]
    n = 32
    nrep = 4000
    variance = 1.5
    figname = "figures/gp_percentages_conditional_marginal_bivariate_density_transposed.png"
    lengthscale = 3.0
    missing_indices1 = [231,426,533,499,450]
    missing_indices2 = [265,430,690,502,490]
    visualize_marginal_bivariate_density_transposed(model_name, missing_indices, missing_indices1, missing_indices2, n, nrep, lengthscale, variance, figname)


visualize_marginal_bivariate_density_transposed_with_variables()