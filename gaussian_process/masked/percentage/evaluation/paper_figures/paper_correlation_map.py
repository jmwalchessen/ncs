...
def visualize_true_and_diffusion_correlation_maps_transposed(missing_indices, n, nrep,
    ...
    for i in range(0, 5):
        ...
        for i, ax in enumerate(grid):
            if((i % 2) == 0):
                counter = counter + 1
                im = ax.imshow(true_cov_images[counter,:,:], cmap='viridis', vmin = 0, vmax = 1,
                                alpha = (1-masks[counter,:,:].astype(float)))
                ax.set_xticks(ticks = [0, 7, 15, 23, 31], labels = np.array([-10,-5,0,5,10]), fontsize = 15)
                if(i==8):
                    ax.set_xticks(ticks = [0, 7, 15, 23, 31], labels = np.array([-10,-5,0,5,10]), fontsize = 15)
            else:
                im = ax.imshow(diffusion_cov_images[counter,:,:], cmap='viridis', vmin = 0, vmax = 1,
                                alpha = (1-masks[counter,:,:].astype(float)))
                if(i == 9):
        ...
        fig.text(x = .23, y = .87, s = "Exact", fontsize = 15)
        fig.text(x = .54, y = .87, s = "NCS", fontsize = 15)
        fig.text(x = .23, y = .91, s = "Proportion U-Net", fontsize = 15)
        plt.tight_layout()
        plt.savefig(figname, dpi = 500)
    
def visualize_true_and_diffusion_correlation_maps_transposed_with_variables():
    ...
    figname = "figures/paper_correlation_map_far_from_observed_transposed.png"
    visualize_true_and_diffusion_correlation_maps_transposed(missing_indices, n, nrep, lengthscale, variance, model_name, figname)