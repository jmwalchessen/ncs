...
def ncsn_conv1x1(in_planes, out_planes, stride=1, bias=True, dilation=1, init_scale=1., padding=0):
  ...
  conv = nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=stride, bias=bias, dilation=dilation,
                   padding=padding)
  #when initializing weights of conv layer, scale them by init_scale, for ncsnpp, init_scale = 0
  init_scale = 1e-10 if init_scale == 0 else init_scale
  conv.weight.data *= init_scale
  conv.bias.data *= init_scale
  return conv

#I don't think this function is used in nscnpp
...
#Not sure what CRPB stands for (looked at source above but no explanation mabe convolutional residual propagation? something)
#CRPBlock is a block of convolutional layers (n_stages long) + pooling layer for each conv layer
# + activation function
class CRPBlock(nn.Module):
  def __init__(self, features, n_stages, act=nn.ReLU(), maxpool=True):
    super().__init__()
    #list of layers I think
    self.convs = nn.ModuleList()
    #number of conv3x3 layers to add = n_stages
    ...
    #Reul is default activation function, might be swish/SiLU for nscnpp
    self.act = act

  def forward(self, x):
    #interesting forward process, activation function first, then go through conv and pooling layers
    #and add x to each of these layers, residual neural network because add identity (i.e. x) to output
    x = self.act(x)
    path = x
...
#conditional CRP block which includes normalization layer and classes feature
class CondCRPBlock(nn.Module):
  ...
  def forward(self, x, y):
    #y is class label, used in normalization layer
    x = self.act(x)
    path = x
    for i in range(self.n_stages):
...
#Create own set of blocks (n_blocks = number of blocks, n_stages is number of conv layers in each block)
class RCUBlock(nn.Module):
  def __init__(self, features, n_blocks, n_stages, act=nn.ReLU()):
    ...
    self.n_stages = n_stages
    self.act = act
#still a residual neural network
  def forward(self, x):
...
#conditional version of RCU block
...
#in_planes needs to be list or tuple i.e. not integer because image has multiple channels maybe?
class MSFBlock(nn.Module):
  def __init__(self, in_planes, features):
    super().__init__()
    assert isinstance(in_planes, list) or isinstance(in_planes, tuple)
    self.convs = nn.ModuleList()
    #features is the number of features in the next conv layer
...
class UpsampleConv(nn.Module):
  def __init__(self, input_dim, output_dim, kernel_size=3, biases=True):
    ...
    self.conv = nn.Conv2d(input_dim, output_dim, kernel_size, stride=1,
                          padding=kernel_size // 2, bias=biases)
    #upscales image from (4*x,h,w) to (x,2*h, 2*w)
...
class ConditionalResidualBlock(nn.Module):
  ...
  #shortcut or conv_shortcut is residual part i.e. adding identity part to output
  def forward(self, x, y):
