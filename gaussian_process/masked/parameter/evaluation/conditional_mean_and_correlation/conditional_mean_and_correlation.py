...
def visualize_empirical_mean_and_diffusion_mean(mask, minX, maxX, minY, maxY, n, variance, lengthscale, observations,
    ...
    for i, ax in enumerate(grid):
        if(i == 0):
            im = ax.imshow(empirical_mean, cmap='viridis', vmin = -2, vmax = 2, alpha = (1-mask).astype(float))
            ax.set_xticks(ticks = [0, 8, 16, 24, 31], labels = np.array([-10,-5,0,5,10]))
            ax.set_yticks(ticks = [0, 8, 16, 24, 31], labels = np.array([-10,-5,0,5,10]))
            ax.set_title("Kriging Mean")
        else:
            diffusion_mean = np.mean(diffusion_samples, axis = (0,1))
            im = ax.imshow(diffusion_mean, cmap = 'viridis', vmin = -2, vmax = 2, alpha = (1-mask).astype(float))
            ax.set_xticks(ticks = [0, 8, 16, 24, 31], labels = np.array([-10,-5,0,5,10]))
            ax.set_yticks(ticks = [0, 8, 16, 24, 31], labels = np.array([-10,-5,0,5,10]))
...
model_name = "model6"
image_name = "ref_image5"
file_name = "model6_variance_1.5_lengthscale_5.0_beta_min_max_01_20_random50_1000"
...
variance = 1.5
lengthscale = 5.0
figname = produce_figure_name(model_name, image_name, "conditional_mean.png")
...
nrep = 1000
visualize_empirical_mean_and_diffusion_mean(mask, minX, maxX, minY, maxY, n, variance, lengthscale, observations,
                                              diffusion_images, figname)