...
sys.path.append((sde_folder + "/op"))
import upfirdn2d as ufdn2d
...
def upsample_conv_2d(x, w, k=None, factor=2, gain=1):
  ...
  return ufdn2d.upfirdn2d_func(x, torch.tensor(k, device=x.device),
...
def conv_downsample_2d(x, w, k=None, factor=2, gain=1):
  ...
  p = (k.shape[0] - factor) + (convW - 1)
  s = [factor, factor]
  x = ufdn2d.upfirdn2d_func(x, torch.tensor(k, device=x.device),
                pad=((p + 1) // 2, p // 2))
...
def downsample_2d(x, k=None, factor=2, gain=1):
  ...
  k = _setup_kernel(k) * gain
  p = k.shape[0] - factor
  #function arguments (input, kernel, up, down, pad)
  return ufdn2d.upfirdn2d_func(x, torch.tensor(k, device=x.device),
