...
class MaskEmbedFC(nn.Module):
  ...
  def __init__(self, number_of_filters: int):
        super(MaskEmbedFC, self).__init__()

        if(number_of_filters == 256):

          layers = [ConvLayer(in_channels = 1, out_channels = 32, kernel = (3,3)),
                    MaxPool2dlayer(kernel = (2,2), padding = (1,1)),
                    ConvLayer(in_channels = 32, out_channels = 16, kernel = (3,3)),
                    MaxPool2dlayer(kernel = (2,2), padding = (1,1)),
                    ConvLayer(in_channels = 16, out_channels = 16, kernel = (3,3)),
                    MaxPool2dlayer(kernel = (2,2), padding = (1,1)),
                    #ConvLayer(in_channels = 4, out_channels = 4, kernel = (3,3)),
                    #MaxPool2dlayer(kernel = (2,2), padding = (1,1)),
                    nn.Flatten()]
          
        if(number_of_filters == 128):
...
class ResnetBlockDDPMpp(nn.Module):
  ...
  def forward(self, x, temb=None):
    ...
    print(self.act)
    act = nn.SiLU()
    print(act(self.GroupNorm_0(x)).shape)
    print(h.shape)
    print(self.Conv_0)
    h = self.Conv_0(h)
    if temb is not None:
      print("a")
      print(self.Dense_0(self.act(temb)).shape)
...
#make sure act = nn.ReLU() or act = nn.SiLU(), involves 3 conv 3x3 layers and groupnorm + residual part
#returns shape (1,nf,n,n)
class SmallResnetBlockDDPMpp(nn.Module):
  """ResBlock adapted from DDPM."""
...
#involves 2 conv layers (1 3x3 and 1 1x1) and up and down sampling
class SmallResnetBlockBigGANpp(nn.Module):
  def __init__(self, act, in_ch, out_ch=None, temb_dim=None, up=False, down=False,
               dropout=0.1, fir=False, fir_kernel=(1, 3, 3, 1),
               skip_rescale=True, init_scale=0.):
    super().__init__()
