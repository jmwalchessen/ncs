...
n <- 28
dim1 <- sample(5:n, replace = FALSE)
dim2 <- sample(5:n, replace = FALSE)
unconditional_images <- unconditional_images[1:4000,,dim1,dim2]
dim(unconditional_images) <- c(4000,576)
uncond_test <- mvn(unconditional_images, subset = NULL, mvnTest = c("mardia"), covariance = TRUE,
                   tol = 1e-25, alpha = 0.75, scale = FALSE)
