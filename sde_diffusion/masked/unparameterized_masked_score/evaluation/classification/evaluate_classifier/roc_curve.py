...
def create_roc_and_auc(number_of_replicates, model_name, evaluation_file_name,
    ...
    roc = roc_curve(true_classes, predicted_class_probabilities.detach().cpu().numpy(), pos_label = 1)
    auc = roc_auc_score(true_classes, predicted_class_probabilities.detach().cpu().numpy())
    return roc, auc  
...
n = 32
crop_size = 2
classifier_name = "classifier12"
num_epochs = 500
classifier = (CNNClassifier()).to("cuda:0")
classifier_file = "standard_cnnclassifier_maxpool_classifier_model6_lengthscale_1.6_variance_0.4_epochs_" + str(num_epochs) + "_parameters.pth"
