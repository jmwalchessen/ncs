...
#mask[:, int(n/4):int(n/4*3), int(n/4):int(n/4*3)] = 0
    ...
    np.save("data/model5/ref_image1/ref_image.npy", ref_img.detach().cpu().numpy().reshape((n,n)))
    np.save("data/model5/ref_image1/diffusion/model5_beta_min_max_01_20_random0_250_" + str(i) + ".npy", conditional_samples)
    np.save("data/model5/ref_image1/partially_observed_field.npy", partially_observed.reshape((n,n)))
    np.save("data/model5/ref_image1/mask.npy", mask.int().detach().cpu().numpy().reshape((n,n)))
    np.save("data/model5/ref_image1/seed_value.npy", np.array([int(seed_value)]))

    plot_spatial_field(ref_img.detach().cpu().numpy().reshape((n,n)), -3, 3, "data/model5/ref_image1/ref_image.png")
    plot_spatial_field((conditional_samples[0,:,:,:]).numpy().reshape((n,n)), -3, 3, "data/model5/ref_image1/diffusion/visualizations/conditional_sample_0.png")
    plot_masked_spatial_field(spatial_field = ref_img.detach().cpu().numpy().reshape((n,n)),
                   vmin = -2, vmax = 2, mask = mask.int().float().detach().cpu().numpy().reshape((n,n)), figname = "data/model5/ref_image1/partially_observed_field.png")
