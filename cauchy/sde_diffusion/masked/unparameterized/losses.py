...
def get_ddpm_loss_fn_old(vpsde, train, reduce_mean=True):
...
def get_ddpm_loss_fn_modified_old(vpsde, train, reduce_mean=True):
  """Legacy code to reproduce previous results on DDPM. Not recommended for new work."""
  assert isinstance(vpsde, VPSDE), "DDPM training only works for VPSDEs."
...
def get_ddpm_loss_fn_same_batch(vpsde, train, reduce_mean=True):
  """Legacy code to reproduce previous results on DDPM. Not recommended for new work."""
  ...
  def loss_fn(model, batch):
    model_fn = mutils.get_model_fn(model, train=train)
    timesteps = (torch.randint(0, vpsde.N, (batch.shape[0],), device=batch.device))
    sqrt_alphas_cumprod = vpsde.sqrt_alphas_cumprod.to(batch.device)
    #this is sigma_t i.e. std
    sqrt_1m_alphas_cumprod = vpsde.sqrt_1m_alphas_cumprod.to(batch.device)
    noise = torch.randn_like(batch)
    ...
    score = model_fn(perturbed_data, timesteps)
    #score=-(1/sigma_t)*noise
    stds = (sqrt_1m_alphas_cumprod[timesteps]).view((batch.shape[0],1,1,1))
    #stds_inverse = sqrt_1m_alphas_cumprod[labels].pow(-1)
    losses = torch.square(torch.mul(stds, score) + noise)
    losses = reduce_op(losses.reshape(losses.shape[0], -1), dim=-1)
    loss = torch.mean(losses)
    return loss
...
def get_ddpm_loss_fn_same_timestep(vpsde, train, reduce_mean = True):
  ...
  def loss_fn(model, batch):
    model_fn = mutils.get_model_fn(model, train=train)
    timesteps = (torch.randint(0, vpsde.N, (1,), device=batch.device)).repeat((batch.shape[0],1))
    timesteps = torch.reshape(timesteps, (batch.shape[0],))
    sqrt_alphas_cumprod = vpsde.sqrt_alphas_cumprod.to(batch.device)
    #this is sigma_t i.e. std
    sqrt_1m_alphas_cumprod = vpsde.sqrt_1m_alphas_cumprod.to(batch.device)
    noise = torch.randn_like(batch)
...
def get_ddpm_loss_fn_same_timestep_weighted(vpsde, train, reduce_mean = True):
  ...
  weights = ([50 for i in range(0, 50)] + [25 for i in range(0, 50)] + [10 for i in range(0, 100)]+ 
             [5 for i in range(0, 300)] + [1 for i in range(0, 500)])

  def loss_fn(model, batch):
    weighted_sampler = torch.utils.data.WeightedRandomSampler(weights, 1,replacement=True)
    model_fn = mutils.get_model_fn(model, train=train)
    timesteps = ((torch.tensor(list(weighted_sampler))).int()).to(batch.device)
    #timesteps = (torch.randint(0, vpsde.N, (1,), device=batch.device)).repeat((batch.shape[0],1))
    timesteps = torch.reshape(timesteps.repeat(batch.shape[0],1), (batch.shape[0],))
    sqrt_alphas_cumprod = vpsde.sqrt_alphas_cumprod.to(batch.device)
    #this is sigma_t i.e. std
    sqrt_1m_alphas_cumprod = vpsde.sqrt_1m_alphas_cumprod.to(batch.device)
    noise = torch.randn_like(batch)
...
def get_ddpm_loss_fn_weighted(vpsde, train, reduce_mean=True):
  """Legacy code to reproduce previous results on DDPM. Not recommended for new work."""
  assert isinstance(vpsde, VPSDE)
  ...
  def loss_fn(model, batch):
    weighted_sampler = torch.utils.data.WeightedRandomSampler(weights, batch.shape[0],replacement=True)
    model_fn = mutils.get_model_fn(model, train=train)
    timesteps = ((torch.tensor(list(weighted_sampler))).int()).to(batch.device)
    sqrt_alphas_cumprod = vpsde.sqrt_alphas_cumprod.to(batch.device)
    #this is sigma_t i.e. std
    ...
    score = model_fn(perturbed_data, timesteps)
    #score=-(1/sigma_t)*noise
    stds = sqrt_1m_alphas_cumprod[timesteps]
    stds = stds.view((batch.shape[0],1,1,1))
    stds_inverse = stds.pow(-1)
    #stds_inverse = sqrt_1m_alphas_cumprod[labels].pow(-1)
    losses = torch.square(torch.mul(stds_inverse, (torch.torch.mul(stds, score) + noise)))
    losses = reduce_op(losses.reshape(losses.shape[0], -1), dim=-1)
    loss = torch.mean(losses)
    return loss
...
def get_smld_loss_fn(vesde, train, reduce_mean=False):
  ...
  def loss_fn(model, batch):
    ...
    target = -torch.mul(noise, (1/sigmas))
    losses = torch.square(score - target)
    #losses = batch_    #target = -batch_mul(noise, (1/sigmas))mul(losses, (1/sigmas**2))
    losses = reduce_op(losses.reshape((losses.shape[0], -1)), axis=-1)
    sigmas = sigmas.view((batch.shape[0]))
    losses = torch.mul(losses, (sigmas**2))
    loss = torch.mean(losses)
    return loss
...
def get_step_fn(sde, train, optimize_fn=None,
  ...
  if continuous:
    ...
    if (isinstance(sde, VPSDE)):
      if masked:
        loss_fn = get_masked_ddpm_loss_fn(sde, train, reduce_mean=reduce_mean) 
      else:
