...
mask <- as.matrix(rbern(n = (n**2), p = .015))
...
condsim <- array((condsim["sim"])[[1]])
print(condsim)
condsim <- as.array(condsim)
np <- import("numpy")
np$save("data/ref_img3/conditional_simulations_10.npy", condsim)
np$save("data/ref_img3/mask01.npy", mask)
np$save("data/ref_img3/ref_img.npy", ref_img)
np$save("data/ref_img3/seed_value.npy", as.array(seed_value))
