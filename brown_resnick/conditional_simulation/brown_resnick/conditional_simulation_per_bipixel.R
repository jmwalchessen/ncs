...
missing_index1 <- as.numeric(args[11])
missing_index2 <- as.numeric(args[12])
...
argsList <- list(range = range, smooth = smooth, nugget = nugget, cov_mod = cov_mod, mask_file_name = mask_file_name,
                 ref_image_name = ref_image_name, neighbors = neighbors, nrep = nrep,
                 missing_index1 = mising_index1, missing_index2 = missing_index2)
produce_mcmc_interpolation_per_bipixel_via_mask(argsList)