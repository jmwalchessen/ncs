...
def produce_true_fcs_ncs_conditional_bivariate_density(n, number_of_replicates, missing_index1,
                                                        missing_index2,
                                                        conditional_fcs_samples,
                                                        conditional_ncs_samples,
                                                        mask, ref_image,
    ...
    matrix_index1 = index_to_matrix_index(missing_index1, n)
    matrix_index2 = index_to_matrix_index(missing_index2, n)
    fcs_biv_density = np.concatenate([(conditional_fcs_samples[:,int(matrix_index1[0]),int(matrix_index1[1])]).reshape((number_of_replicates,1)),
                                       (conditional_fcs_samples[:,int(matrix_index2[0]),int(matrix_index2[1])]).reshape((number_of_replicates,1))], axis = 1)
    ncs_biv_density = np.concatenate([(conditional_ncs_samples[:,0,int(matrix_index1[0]),int(matrix_index1[1])]).reshape((number_of_replicates,1)),
                                       (conditional_ncs_samples[:,0,int(matrix_index2[0]),int(matrix_index2[1])]).reshape((number_of_replicates,1))], axis = 1)
    ...
    for j in range(observed_indices.shape[0]):
        rect = Rectangle(((observed_indices[j,1]-.55), (observed_indices[j,0]-.55)), width=1, height=1, facecolor='none', edgecolor='r')
        axs[0].add_patch(rect)
    axs[0].imshow(ref_image.reshape((n,n)), vmin = -2, vmax = 4)
    axs[0].plot(matrix_index1[1], matrix_index1[0], "rx", markersize = 20, linewidth = 20)
    axs[0].plot(matrix_index2[1], matrix_index2[0], "rx", markersize = 20, linewidth = 20)
    sns.kdeplot(x = fcs_biv_density[:,0], y = fcs_biv_density[:,1],
                ax = axs[1], color = "purple")
...
def produce_multiple_true_fcs_ncs_conditional_bivariate_densities_with_variables():
    ...
    for m in ms:
        for i,range_value in enumerate(range_values):
            for missing_index1 in missing_indices1:
                ...
                missing_indices2 = (np.array((np.where(mask.reshape((n**2)) == 1)))).reshape((m)).tolist()
                for missing_index2 in missing_indices2:
                    figname = (ref_folder + "/bivariate_density/bivariate_density_" + str(missing_index1) + "_" + str(missing_index2) + ".png")
                    produce_true_fcs_ncs_conditional_bivariate_density(n,
                                                        number_of_replicates, missing_index1,
                                                        missing_index2,
                                                        conditional_fcs_samples,
                                                        conditional_ncs_samples,
...
produce_multiple_true_fcs_ncs_conditional_bivariate_densities_with_variables()