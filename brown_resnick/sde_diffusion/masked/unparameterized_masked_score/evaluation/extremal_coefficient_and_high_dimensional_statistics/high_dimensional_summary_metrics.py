...
def compute_minimum_summary_metric(ncs_images_file, true_images_file, fcs_images_file, figname, nrep, n):
    ...
    fcs_mins = np.min(fcs_images.reshape((nrep, n**2)), axis = 1)

    fig, ax = plt.subplots()
    ncspdd = pd.DataFrame(ncs_mins, columns = None)
    truepdd = pd.DataFrame(true_mins, columns = None)
    fcspdd = pd.DataFrame(fcs_mins, columns = None)
    sns.kdeplot(data = truepdd, palette=['blue'], ax = ax)
    sns.kdeplot(data = ncspdd, palette=['orange'], ax = ax)
    sns.kdeplot(data = fcspdd, palette=['purple'], ax = ax)
    ax.legend(labels = ['true', 'NCS', 'FCS'])
...
def compute_quantile_summary_metric(ncs_images_file, true_images_file, fcs_images_file, figname, nrep, n, q):
    ...
    fcs_quants = np.quantile(fcs_images.reshape((nrep, n**2)), q = q, axis = 1)

    fig, ax = plt.subplots()
    ncspdd = pd.DataFrame(ncs_quants, columns = None)
    truepdd = pd.DataFrame(true_quants, columns = None)
    fcspdd = pd.DataFrame(fcs_quants, columns = None)
...
fcs_images_file = "data/fcs/unconditional_fcs_range_3.0_smooth_1.5_nugget_1e5_obs_1_4000.npy"
nrep = 4000
n = 32
compute_maximum_summary_metric(ncs_images_file, true_images_file, fcs_images_file, figname, nrep, n)
figname = "high_dimensional_summary_statistics/ncs/model4/minimum_summary_statistics_ncs_vs_true_4000_random" + str(p) + ".png"
compute_minimum_summary_metric(ncs_images_file, true_images_file, fcs_images_file, figname, nrep, n)
q = .01
...
compute_quantile_summary_metric(ncs_images_file, true_images_file, fcs_images_file, figname, nrep, n, q)
