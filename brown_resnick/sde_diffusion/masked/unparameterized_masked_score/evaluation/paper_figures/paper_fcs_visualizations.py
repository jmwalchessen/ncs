...
def visualize_fcs_ncs_multiple_obs_fixed_range(evaluation_folder, range_value, obs_numbers, n, figname, folder_name,
    ...
    for i, ax in enumerate(grid):
        if(i < 5):
            ...
            im = ax.imshow(ncs_images[(i%5),190,:,:], cmap='viridis', vmin = -2, vmax = 6)
            ax.set_xticks(ticks = [0, 8, 16, 24, 31], labels = np.array([-10,-5,0,5,10]))
            ax.set_yticks(ticks = [0, 8, 16, 24, 31], labels = np.array([-10,-5,0,5,10]))
            observed_indices = np.argwhere(masks[(i%5),:,:] > 0)
            for j in range(observed_indices.shape[0]):
...
def visualize_fcs_ncs_multiple_ranges_fixed_obs(evaluation_folder, range_values, obs, n,
    ...
    for i, ax in enumerate(grid):
        if(i < 5):
            ...
            im = ax.imshow(fcs_images[(i%5),6,:,:], cmap='viridis', vmin = -2, vmax = 6)
            ax.set_xticks(ticks = [0, 8, 16, 24, 31], labels = np.array([-10,-5,0,5,10]),
                          fontsize = 15)
            ...
            im = ax.imshow(ncs_images[(i%5),19,:,:], cmap='viridis', vmin = -2, vmax = 6)
            if((i % 2) == 0):
...
def visualize_fcs_ncs_multiple_ranges_fixed_obs_with_variables():
    ...
    for i,obs in enumerate(obs_numbers):
        figname = "paper_fcs_ncs_conditional_visualizations_obs_" + str(obs) + ".png"
        folder_name = "figures/one_to_seven"
