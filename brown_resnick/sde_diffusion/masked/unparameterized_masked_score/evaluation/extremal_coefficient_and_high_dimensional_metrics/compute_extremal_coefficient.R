...
compute_true_extremal_coefficient_with_variables <- function()
{
  ...
  compute_true_extremal_coefficient(number_of_replicates, number_of_replicates_per_call,
                                    range, smooth, nbins, ext_file, true_image_file, n)
}
...
compute_fcs_extremal_coefficient_with_variables <- function()
{
  ...
  { 
    m <- ms[i]
    fcs_file <- paste(paste("data/fcs/processed_unconditional_fcs_range_3.0_smooth_1.5_nugget_1e5_obs",
                            as.character(m), sep = "_"), "4000.npy", sep = "_")
    fcs_ext_file <- paste(paste("data/fcs/extremal_coefficient_fcs_range_3.0_smooth_1.5_nugget_1e5_obs", as.character(m),
                                sep = "_"), "4000.npy", sep = "_")
    compute_fcs_extremal_coefficient(number_of_replicates, range, smooth, nbins, fcs_file, fcs_ext_file, n, m)
  }
}
