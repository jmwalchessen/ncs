    ...
    missing_index2 <- indices2[i]
      if((missing_index1 != missing_index2))
      {
        y <- produce_bivariate_lcs_per_pixel_via_mask_interrupted(n, range, smooth, nugget, cov_mod, mask_file_name,
                                                                ...
                                                                missing_index2)
        current_condsim_file <- paste(paste(paste(paste(condsim_file_name, as.character(p), sep = ""), as.character((missing_index1-1)), sep = "_"),
...
produce_bivariate_local_conditional_simulation_multiple_references <- function(indices1, indices2, n, range_value,
                                                                               smooth, nugget, cov_mod, neighbors,
                                                                               nrep, condsim_file_name, model_folder,
                                                                               ref_image_indices, ps)
{
  for(i in 1:length(ref_image_indices))
  {
    ...
    produce_bivariate_lcs_for_multiple_pixels(indices1, indices2, n, range_value, smooth, nugget, cov_mod, mask_file_name,
                                              ref_image_name, neighbors, nrep, current_condsim_file_name, ps[i])
  }
}