...
def produce_generated_and_univariate_lcs_marginal_density_multiple_ranges(n, missing_indices, range_values,
    ...
    univariate_lcs_images = np.zeros((len(range_values),nrep,n,n))
    generated_marginal_density = np.zeros((len(range_values),nrep))
    lcs_marginal_density = np.zeros((len(range_values),nrep))
