...
def generate_ncs_images(ref_folder, vpsde, score_model, range_value,
    ...
    ref_image = load_npfile((ref_folder + "/ref_image.npy"))
    print(ref_image)
    ...
    for i in range(calls):
        print(i)
        ncs_images[i*batches_per_call:(i+1)*batches_per_call,:,:] = ((multiple_posterior_sample_with_p_mean_variance_via_mask(vpsde, score_model, device, mask,
                                                                                                                              masked_image, n, range_value, smooth_value)).detach().cpu().numpy()).reshape((batches_per_call,n,n))
...
def generate_ncs_images_multiple_ranges(vpsde, score_model, range_values,
    ...
    for i in range(len(range_values)):
        ...
        ncs_images_file = (ref_folder + "/ncs/ncs_images_range_" + str(range_values[i]) +
