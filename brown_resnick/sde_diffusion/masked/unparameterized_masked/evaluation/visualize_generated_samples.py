...
score_model.load_state_dict(th.load((home_folder + "/trained_score_models/vpsde/model1_beta_min_max_01_20_1000_random050_log_parameterized_mask.pth")))
score_model.eval()
...
def visualize_observed_and_generated_samples(observed, mask, diffusion1, diffusion2, n, figname):
    ...
    im = grid[0].imshow(observed.detach().cpu().numpy().reshape((n,n)), vmin=-2, vmax=4)
    grid[0].set_title("Observed")
    grid[1].imshow(observed.detach().cpu().numpy().reshape((n,n)), vmin=-2, vmax=4,
                   alpha = mask.detach().cpu().numpy().reshape((n,n)))
    grid[1].set_title("Partially Observed")
    grid[2].imshow(diffusion1.detach().cpu().numpy().reshape((n,n)), vmin=-2, vmax=4)
    grid[2].set_title("Generated")
    grid[3].imshow(diffusion2.detach().cpu().numpy().reshape((n,n)), vmin=-2, vmax=4)
...
seed_value = 234324
br_samples = (log_transformation(np.load("brown_resnick_samples_500.npy"))).reshape((number_of_replicates,1,n,n))
...
for i in range(0,10):
    ...
    unmasked_y = (torch.from_numpy(br_samples[0,:,:,:])).to(device)
    ...
    figname = ("visualizations/models/model1/random50_log_observed_and_generated_samples_" + str(i) + ".png")
