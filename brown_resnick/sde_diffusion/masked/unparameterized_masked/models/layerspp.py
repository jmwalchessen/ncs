...
class CondResnetBlockBigGANpp(nn.Module):
  def __init__(self, act, in_ch, mask_dim, out_ch=None, temb_dim=None, up=False, down=False,
               dropout=0.1, fir=False, fir_kernel=(1, 3, 3, 1),
               skip_rescale=True, init_scale=0.):
    super().__init__()
...
mask_dim = (1,1,32,32)
act = nn.ReLU()
crbbg = CondResnetBlockBigGANpp(act, in_ch, mask_dim, out_ch=None, temb_dim=None, up=False, down=False,
               dropout=0.1, fir=False, fir_kernel=(1, 3, 3, 1),
               skip_rescale=True, init_scale=0.)
...
p = .5
device = "cuda:0"
mask = (torch.bernoulli(p*torch.ones((1,1,32,32)))).to(device)
crbbg.forward(x,mask,temb = None)
"""