...
def produce_generated_and_bivariate_lcs_density_multiple_percentages(n, missing_indices1, missing_indices2,
                                                                    ps, bivariate_lcs_file,
                                                                    figname, nrep):

    masks = np.zeros((len(ps),n,n))
    ref_images = np.zeros((len(ps),n,n))
    ncs_images = np.zeros((len(ps),nrep, n,n))
    generated_bivariate_density = np.zeros((len(ps),nrep,2))
    lcs_bivariate_density = np.zeros((len(ps),nrep,2))
...
figname = "figures/br_percentage_lcs_vs_ncs_conditional_bivariate_density.png"
nrep = 4000
ps = [.01,.05,.1,.25,.5]
bivariate_lcs_file = "bivariate_lcs_4000_neighbors_7_nugget_1e5"
produce_generated_and_bivariate_lcs_density_multiple_percentages(n, missing_indices1, missing_indices2,
                                                                 ps, bivariate_lcs_file,
                                                                 figname, nrep)