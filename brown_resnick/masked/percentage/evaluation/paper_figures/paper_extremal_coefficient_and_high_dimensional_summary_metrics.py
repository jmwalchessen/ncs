...
def visualize_ncs_and_true_extremal_coefficient_and_high_dimensional_summary_metrics_multiple_percentages_small(range_value, smooth,
                                                                                                          ps, bins, figname, nrep, n):
    ...
    for i in range(12):
        ax = fig.add_subplot(spec[i])
        if(i < 3):
            ext_coeff = 2-extremal_matrices[obs_indices[i%3],:,2]
            print(ext_coeff)
            ax.set_ylim(0,1)
            ax.set_xlim(0,20)
            ncs_ext_coeff = 2-ncs_extremal_matrices[obs_indices[i%3],:,2]
            ax.plot(h, ext_coeff, "blue")
            ax.plot(h, ncs_ext_coeff, "orange", linestyle = "dashed")
            if(i == 0):
                ...
                ax.set_title(r"$\rho = .01$", fontsize = 15)
            if((i == 2)):
                ax.set_xticks(ticks = [0,10,20], labels = np.array([0,10,20]), fontsize = 15)
                ax.set_title(r"$\rho = .5$", fontsize = 15)
            else:
                ax.set_xticks([])
            if(i == 1):
                ax.set_title(r"$\rho = .1$", fontsize = 15)
            if(i != 0):
                ax.set_yticks([])
    ...
    fig.text(0.45, .69, "Minimum", fontsize = 18)
    fig.text(0.45, .48, "Maximum", fontsize = 18)
    fig.text(0.35, .28, "Absolute Summation", fontsize = 18)
    fig.text(0.38, .93, "Proportion U-Net", fontsize = 18)
    plt.tight_layout()
    ...
    for i in range(20):
        ...
        if(i < 5):
            ...
            if((i != 5)):
                ...
                ax.set_yticks(ticks = [0.,1.,2.], labels = np.array([0.,1.,2.]), fontsize = 15)
            if((i != 7)):
...
def visualize_ncs_fcs_and_true_extremal_coefficient_and_high_dimensional_summary_metrics_1_3_7_range_1(range_value, smooth,
                                                                                                      bins, figname, nrep, n):
    
    obs_numbers = [1,3,7]
    extremal_matrices = np.zeros((len(obs_numbers), (bins+1),3))
    ncs_extremal_matrices = np.zeros((3, (bins+1),3))
    fcs_extremal_matrices = np.zeros((3, (bins+1),3))
    ncs_images = np.zeros((len(obs_numbers),nrep,n,n))
    fcs_images = np.zeros((len(obs_numbers),nrep,n,n))
    true_images = np.zeros((len(obs_numbers),nrep,n,n))
    ncs_abs_summation = np.zeros((len(obs_numbers),nrep))
    ...
    for i in range(12):
        ...
        if(i < 3):
            ...
            if(i == 0):
                ax.set_xlabel("Distance Lag (h)", fontsize = 18)
                #ax.set_ylabel("2-Extremal Coeff.", fontsize = 18)
                ax.set_title("1 Obs.", fontsize = 20)
                ax.set_yticks([0., .25, .5, .75], [0., .25, .5, .75], fontsize = 17)
                ax.set_xticks([])
                ax.legend(labels = ['true', 'NCS', 'FCS'], fontsize = 14)
        
            else:
                ax.set_xlabel("")
                ax.set_ylabel("")
                ax.set_yticks([])
                ax.set_title((str(obs_numbers[i]) + " Obs."), fontsize = 20)
                if((i != 1)):
                    ax.set_xticks([0,10,20], [0,10,20], fontsize = 17)
                else:
                    ax.set_xticks([])
        
        elif(i < 6):
            ncspdd = pd.DataFrame(ncs_mins[(i%3),:], columns = None)
            fcspdd = pd.DataFrame(fcs_mins[(i%3),:], columns = None)
            truepdd = pd.DataFrame(true_mins[(i%3),:], columns = None)
            sns.kdeplot(data = truepdd, palette = ['blue'], ax = ax, legend = False)
            sns.kdeplot(data = ncspdd, palette =['orange'], ax = ax, legend = False, linestyle = "dashed")
            sns.kdeplot(data = fcspdd, palette =['purple'], ax = ax, legend = False, linestyle = "dashed")
            ax.set_xlim((-35,1))
            ...
            if((i != 3)):
                ...
                ax.set_yticks(ticks = [0.,1.,2.], labels = np.array([0.,1.,2.]), fontsize = 17)
            if((i != 4)):
                ax.set_xticks(ticks = [-30,-20,-10,0], labels = np.array([-30,-20,-10,0]), fontsize = 17)
            else:
                ax.set_xticks([])
        
        elif(i < 9):
            ncspdd = pd.DataFrame(ncs_maxs[(i%3),:], columns = None)
            fcspdd = pd.DataFrame(fcs_maxs[(i%3),:], columns = None)
            truepdd = pd.DataFrame(true_maxs[(i%3),:], columns = None)
            sns.kdeplot(data = truepdd, palette = ['blue'], ax = ax, legend = False)
            sns.kdeplot(data = ncspdd, palette =['orange'], ax = ax, legend = False, linestyle = "dashed")
            sns.kdeplot(data = fcspdd, palette =['purple'], ax = ax, legend = False, linestyle = "dashed")
            ax.set_xlim((0,20))
            ...
            if(i != 6):
                ...
                ax.set_yticks(ticks = [0.,.2,.4], labels = np.array([0.,.2,.4]), fontsize = 17)
            if(i == 7):
                ax.set_xticks([])
            elif(i == 8):
                ax.set_xticks(ticks = [10,20], labels = np.array([10,20]), fontsize = 17)
            else:
                ax.set_xticks(ticks = [0,10], labels = np.array([0,10]), fontsize = 17)


        else:
            ncspdd = pd.DataFrame(ncs_abs_summation[(i%3),:], columns = None)
            fcspdd = pd.DataFrame(fcs_abs_summation[(i%3),:], columns = None)
            truepdd = pd.DataFrame(true_abs_summation[(i%3),:], columns = None)
            sns.kdeplot(data = truepdd, palette = ['blue'], ax = ax, legend = False)
            sns.kdeplot(data = ncspdd, palette =['orange'], ax = ax, legend = False, linestyle = "dashed")
            sns.kdeplot(data = fcspdd, palette =['purple'], ax = ax, legend = False, linestyle = "dashed")
            ax.set_xlim((0,12000))
            ...
            if(i != 9):
                ax.set_yticks([])
                ax.set_xticks(ticks = [0,8000], labels = np.array([0,8000]), fontsize = 17)
            else:
                ax.set_yticks(ticks = [0.,.001,.002], labels = np.array([0,.001,.002]), fontsize = 17)
                ax.set_xticks(ticks = [0,8000], labels = np.array([0,8000]), fontsize = 17)
        
    fig.text(0.44, .69, "Minimum", fontsize = 18)
    fig.text(0.44, .48, "Maximum", fontsize = 18)
    fig.text(0.33, .28, "Absolute Summation", fontsize = 18)
    fig.text(0.01,.77, r"$\chi(h)$", rotation = "vertical", fontsize = 18)
    fig.text(0.46, .95, r"$\lambda = 1$", fontsize = 20)
    #fig.text(0.29, .95, "Extremal Coerrelation", fontsize = 18)
    plt.tight_layout()
    plt.savefig(figname, dpi = 500)


def visualize_ncs_fcs_and_true_extremal_coefficient_and_high_dimensional_summary_metrics_1_3_7_range_5(range_value, smooth,
                                                                                                      bins, figname, nrep, n):
        ...
        if(i < 3):
            ...
            if(i != 3):
                ...
                if(i==4):
                    ...
                    ax.set_xticks([-2,0], [-2,0], fontsize = 17)  
            else:
                #ax.legend(labels = ['true', 'NCS', 'FCS'], fontsize = 12)
                ax.set_yticks([0.,.5,1.,1.5], [0.,.5,1.,1.5], fontsize = 17)
                ax.set_xticks([-2,0], [-2,0], fontsize = 17)  
            ...
            if(i != 6):
                ax.set_yticks([])
                if(i != 7):
                    ax.set_xticks([5,10,15], [5,10,15], fontsize = 17)
                else:
                    ax.set_xticks([])
            else:
                ax.set_yticks([0.,.2,.4], [0.,.2,.4], fontsize = 17)
                ax.set_xticks([0,5,10], [0,5,10], fontsize = 17)
                #ax.legend(labels = ['true', 'NCS', 'FCS'], fontsize = 12)
            ...
            ax.set_xlabel("")
            ax.set_ylabel("")
            ax.set_xticks([0,2000,4000], [0,2000,4000], fontsize = 17)
            if(i != 9):
                #ax.set_xticks([])
                ax.set_yticks([])
            else:
                #ax.legend(labels = ['true', 'NCS', 'FCS'], fontsize = 12)
                ax.set_yticks([0.,0.001,.002], [0.,0.001,.002], fontsize = 17)
        
    fig.text(0.44, .69, "Minimum", fontsize = 18)
    fig.text(0.44, .48, "Maximum", fontsize = 18)
    fig.text(0.34, .28, "Absolute Summation", fontsize = 18)
    fig.text(0.01,.77, r"$\chi(h)$", rotation = "vertical", fontsize = 18)
    fig.text(0.46, .95, r"$\lambda = 5$", fontsize = 20)
    #fig.text(0.29, .95, "Extremal Coerrelation", fontsize = 18)
    plt.tight_layout()
    plt.savefig(figname, dpi = 500)


def visualize_ncs_and_true_extr_coeff_and_high_dimensional_summary_metrics_multiple_percentages_with_variables():
        
        range_value = 3.
        smooth = 1.5
        ps = [.01,.05,.1,.25,.5]
        bins = 100
        figname = "figures/paper_ncs_vs_true_extremal_coefficient_and_high_dimensional_summary_metrics_small.png"
        nrep = 4000
        n = 32
