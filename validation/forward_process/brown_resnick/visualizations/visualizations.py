...
def forward_process(sde, br_sample):

    noise = th.randn_like(br_sample)
    perturbed_data = sde.sqrt_alphas_cumprod[(sde.N-1), None, None, None] * br_sample + \
                     sde.sqrt_1m_alphas_cumprod[(sde.N-1), None, None, None] * noise
    return perturbed_data

def visualize(sde, br_sample, n):

    perturbed_data = forward_process(sde, br_sample)
    perturbed_data = perturbed_data.numpy().reshape((n,n))
    plt.imshow(perturbed_data, vmin = -2, vmax = 2)
    plt.show()

def plot_marginal_density(sde, br_samples, n, number_of_replicates, matrix_index, figname):
    fig, ax = plt.subplots(1)
    br_samples = th.from_numpy(log_transformation(br_samples.reshape((number_of_replicates,n,n))))
    perturbed_data = forward_process(sde, br_samples)
...
def plot_bivariate_density(sde, br_samples, n, number_of_replicates, matrixindex1, matrixindex2, figname):
...
def plot_gp_marginal_density(sde, n, number_of_replicates, matrix_index, figname):
    fig, ax = plt.subplots(1)
    ...
    perturbed_data = forward_process(sde, gp_samples)
    marginal_density = perturbed_data[:,0,matrix_index[0],matrix_index[1]]
    pdd = pd.DataFrame(marginal_density, columns = ['true'])
    sns.kdeplot(data = pdd["true"], palette = ["orange"], bw_adjust = 1, ax = ax)
    plt.savefig(figname)

def plot_gp_bivariate_density(sde, n, number_of_replicates, matrixindex1, matrixindex2, figname):
...
matrix_index1 = (8,16)
matrix_index2 = (8,17)
figname = "bivariate_density_gp_1000_8_16_8_17.png"
#plot_bivariate_density(sdevp, br_samples, n, number_of_replicates, matrix_index1, matrix_index2, figname)
sdevp = VPSDE(beta_min=0.1, beta_max=25, N = 250)
plot_gp_bivariate_density(sdevp, n, number_of_replicates, matrix_index1, matrix_index2, figname)
matrix_index = (9,9)
figname = "marginal_density_gp_1000_8_16.png"
plot_gp_marginal_density(sdevp, n, number_of_replicates, matrix_index, figname)