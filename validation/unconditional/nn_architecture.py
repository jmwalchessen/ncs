...
class DenseLayer(nn.Module):
    def __init__(self, in_features: int, out_features: int) -> None:
        ...
        lin = nn.Linear(in_features = in_features, out_features = out_features,
                                        bias = True)
        self.dense = nn.Sequential(lin, nn.ReLU())
...
class CNNClassifier(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        layers = [ConvLayer(in_channels = 1, out_channels = 128, kernel = (3,3)),
                  ...
                  MaxPool2dlayer(kernel = (2,2), padding = (1,1)),
                  ConvLayer(in_channels = 128, out_channels = 64, kernel = (3,3)),
                  MaxPool2dlayer(kernel = (2,2), padding = (1,1)),
                  ConvLayer(in_channels = 64, out_channels = 16, kernel = (3,3)),
                  MaxPool2dlayer(kernel = (2,2), padding = (1,1)),
                  nn.Flatten(),
    ...
    def partial_forward(self, x:torch.Tensor, i: int) -> torch.Tensor:
        partial_layers = nn.Sequential(*self.layers[:i])
        return partial_layers(x)