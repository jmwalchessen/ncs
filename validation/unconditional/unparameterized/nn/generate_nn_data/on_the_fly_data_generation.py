...
def generate_gaussian_process(minX, maxX, minY, maxY, n, variance, lengthscale, number_of_replicates,
    ...
    y_matrix = (np.flip(L, np.transpose(z_matrix)))
    
    gp_matrix = np.zeros((number_of_replicates,n,n))
    for i in range(0, y_matrix.shape[1]):
        gp_matrix[i,:,:] = y_matrix[:,i].reshape((n,n))
    return gp_matrix


#first column of parameter_matrix is variance
def generate_images_given_parameters_on_the_fly(minX, maxX, minY, maxY, n, parameter_matrix, number_of_replicates,
                             seed_values):
    
    x_train_images = np.empty((0, n, n))
    ...
    for i in range(0, parameter_matrix.shape[0]):
        ...
        current_parameters = np.repeat((parameter_matrix[i,:]).reshape((1,2)),
                                       repeats = number_of_replicates, axis = 0)
        x_train_parameters = np.concatenate([x_train_parameters,
                                             current_parameters], axis = 0)
        
    return x_train_images, x_train_parameters

def generate_data_on_the_fly(minX, maxX, minY, maxY, n, number_of_paramters, number_of_replicates
                            , seed_values, lhs_seed):
    
    lhs_sampler = parameter_matrix = LHS(d = 2, seed = lhs_seed)
    #scale to [0,2]x[0,2]
    parameter_matrix = 2*lhs_sampler.random(n = number_of_paramters)
    x_train_images, x_train_parameters = generate_images_given_parameters_on_the_fly(minX, maxX, minY,
                                                                                     maxY, n, parameter_matrix,
                                                                                     number_of_replicates, seed_values)
    return x_train_images, x_train_parameters
