...
def generate_first_class_data(minX, maxX, minY, maxY, n, variance, lengthscale, number_of_replicates_per_mask,
                             seed_value):
...
def generate_masks(number_of_masks, p, n, seed_value):
    np.random.seed(seed_value)
    mask_matrix = (torch.bernoulli(p*torch.ones((number_of_masks,1,n,n)))).numpy()
    return mask_matrix

def generate_second_class_from_first_class(first_class_images, number_of_masks, p, n, seed_value):
    mask_matrix = generate_masks(number_of_masks, p, n, seed_value)
