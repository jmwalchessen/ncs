...
def get_training_and_evaluation_dataset_per_mask(number_of_replicates, number_of_evaluation_replicates,
                                                 batch_size, eval_batch_size, seed_values, rho,
                                                 alpha, mask):
    minX = minY = -10
    maxX = maxY = 10
    n = (32**2)
    train_images = generate_spatial_arch_process(rho, alpha, seed_values[0],
                                                  number_of_replicates, n)

    train_dataset = CustomSpatialImageandSingleMaskDataset(train_images)
    train_dataloader = DataLoader(train_dataset, batch_size = batch_size, shuffle = True)
    eval_images = generate_spatial_arch_process(alpha, rho, seed_values[1],
                                                 number_of_evaluation_replicates, n)
...
def get_training_and_evaluation_image_datasets_per_mask(number_of_replicates_per_mask,
                                                        ...
                                                        seed_values, rho, alpha, image_batch_size,
                                                        eval_batch_size):
    ...
    eval_images = generate_spatial_arch_process(rho, alpha, seed_values[0],
...
def get_training_and_evaluation_mask_and_image_datasets_per_mask(number_of_random_replicates, 
                                                                 random_missingness_percentages, 
                                                                 number_of_evaluation_random_replicates,
                                                                 batch_size, eval_batch_size, rho, alpha,
                                                                 seed_values):
    ...
    train_images, eval_images = generate_train_and_evaluation_spatial_arch_process(rho, alpha, 
                                                                                   seed_values[0],
