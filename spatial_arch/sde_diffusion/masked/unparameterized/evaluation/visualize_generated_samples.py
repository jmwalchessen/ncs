...
import os
import sys
from spatial_arch_data_generation import *
...
score_model.load_state_dict(th.load((home_folder + "/trained_score_models/vpsde/model1_beta_min_max_01_25_250_050.pth")))
score_model.eval()
...
for i in range(0,10):
    ...
    unmasked_y = (torch.from_numpy(generate_spatial_arch_process(rho, alpha, seed_value, number_of_replicates, n))).to(device)
    unmasked_y = unmasked_y.reshape((1,1,n,n))
    print(torch.max(unmasked_y))
    ...
    figname = ("visualizations/models/model1/observed_and_generated_samples_" + str(p) + "_" + str(i) + ".png")
